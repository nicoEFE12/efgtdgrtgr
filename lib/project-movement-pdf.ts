...
export function generateProjectMovementReceipt(
  movement: any, 
  projectName: string = "",
  clientName: string = "",
  contractNumber: string = "",
  withComprobante: boolean = false
) {
  const pdf = new jsPDF();
  ...
  // Header with company info (no logo, just text)
  ...
  pdf.setFont("helvetica", "bold");
  pdf.text(
    movement.type === "ingreso" || movement.type === "transferencia_in" || movement.type === "cobro" ? "RECIBO DE INGRESO" : "COMPROBANTE DE EGRESO",
    pageWidth / 2,
    yPos,
    { align: "center" }
  );
  ...
  pdf.setFont("helvetica", "normal");
  pdf.text(contractNumber || "-", margin + 35, yPos);
  ...
  pdf.setFont("helvetica", "bold");
  pdf.text("CATEGORÍA:", margin + 5, yPos);
  pdf.setFont("helvetica", "normal");
  ...
  // Amount box
  const isIncome = movement.type === "ingreso" || movement.type === "transferencia_in" || movement.type === "cobro";
  ...
  pdf.setFontSize(16);
  pdf.text(formatCurrency(Number(movement.amount), movement.currency || "ARS"), pageWidth - margin - 5, yPos + 14, { align: "right" });
  ...
  if (withComprobante) {
    ...
    pdf.setFontSize(10);
    pdf.setFont("helvetica", "normal");
    pdf.text(`Obra: ${projectName || "_____________________________"}`, margin, yPos);
    yPos += 8;
    pdf.text(`La suma de: ${formatCurrency(Number(movement.amount), movement.currency || "ARS")}`, margin, yPos);
    yPos += 6;
    pdf.setFontSize(9);
    const amountInText = numberToText(Number(movement.amount), movement.currency || "ARS");
    ...
    yPos += 8;
    pdf.text(`Contrato N°: ${contractNumber || "_____________________________"}`, margin, yPos);
    yPos += 8;
    pdf.text(`Fecha: ${new Date(movement.date).toLocaleDateString("es-AR")}`, margin, yPos);
...
export function previewProjectMovementPDF(
  movement: any, 
  projectName: string = "",
  clientName: string = "",
  contractNumber: string = "",
  ...
  const pdf = generateProjectMovementReceipt(movement, projectName, clientName, contractNumber, withComprobante);
  const pdfBlob = pdf.output("blob");
  const url = URL.createObjectURL(pdfBlob);
  window.open(url, "_blank");
}

export function downloadProjectMovementPDF(
  movement: any, 
  projectName: string = "",
  clientName: string = "",
  contractNumber: string = "",
